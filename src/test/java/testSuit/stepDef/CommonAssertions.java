...
public class CommonAssertions {
    ...
    @Then("retrieve the response object from response context with key {string} and validate response code should be {string}")
    public void should_have_response_code(String contextKey , String expectedResCode) {
        String actualValueResponseStatusCode =
    ...
    @SneakyThrows
    @Then("retrieve the response object from response context with key {string} and response body should be {string} ignoring all extra fields")
    public void should_have_response_body_with_ignoring_all_extra_fields(String contextKey , String expectedValue) {
        String expectedRes =
                new String(Files.readAllBytes(Paths.get(System.getProperty("user.dir")+"/src/test/resources/testData"+expectedValue)));
        String actualRes = testContext.getNewResponseContext().get(contextKey).getBody().asString();
        JSONAssert.assertEquals(expectedRes, actualRes, false);
